...
function MineOSInterface.createWidgets()
	...
	MineOSContextMenu:addItem("MineOS EFI: v.5.0");
	local OpenALMenu = MineOSInterface.mainContainer.menu:addChild(GUI.menu(0, 0, 80, 0xE1E1E1, 0x0, 0x3366CC, 0xFFFFFF, nil))
	--local MineOSContextMenu = MineOSInterface.mainContainer.menu:addContextMenu("MineOS", 0x000000)
	OpenALMenu:addItem("OpenAL").onTouch = function()
		MineOSInterface.safeLaunch(MineOSPaths.applications .. "OpenAL.app/Main.lua", "")
	end
	
	OpenALMenu:addItem(MineOSCore.localization.aboutSystem).onTouch = function()
		...
		local lines = {
			...
			" ",
			"OpenAL",
			"Developer: Artyemlom, vk.com/leaderartyom",
			"v.1.00",
		}
	...
	OpenALMenu:addItem("App Market").onTouch = function()
		MineOSInterface.safeLaunch(MineOSPaths.applications .. "App Market.app/Main.lua", "")
	end
	
	OpenALMenu:addItem(MineOSCore.localization.reboot).onTouch = function()
		MineOSNetwork.broadcastComputerState(false)
		require("computer").shutdown(true)
	end

	OpenALMenu:addItem(MineOSCore.localization.shutdown).onTouch = function()
		MineOSNetwork.broadcastComputerState(false)
		require("computer").shutdown()
	end
	
	OpenALMenu:addItem(MineOSCore.localization.returnToShell).onTouch = function()
		MineOSNetwork.broadcastComputerState(false)
